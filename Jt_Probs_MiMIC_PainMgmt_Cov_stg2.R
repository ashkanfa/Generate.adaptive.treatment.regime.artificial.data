...
#Cov_Stage2 <- read.csv("NewCombinedbinaryconfoundingvar_stage1.csv",header=TRUE)
  ...
  for(j in 1:t)
  {
    ...
    for(k in 1:n)
    {
      ...
      if(idx==0)
      {
        ...
        prob.mimic.stg2 <- rbind(probtemp,prob.mimic.stg2,deparse.level = 0)
      }else{
        probtemp <- phat.mimic[k]
        prob.mimic.stg2 <- rbind(probtemp,prob.mimic.stg2,deparse.level = 0)
      } 
    }
    predprobs.mimic <- cbind(predprobs.mimic,prob.mimic.stg2,deparse.level = 0)#bind the probabilities in pred probs for IPTW calculations
  }
...
prob.mimic.stg2<-as.matrix(prob.mimic.stg2)
colnames(prob.mimic.stg2)=c("wt.stg2")
write.csv(prob.mimic.stg2,file="wt_stg2.csv",row.names = F)
